...
if (empty($card_color_id) || empty($route_id) || empty($driver_id) || empty($vehicle_id) || empty($cashier_id)) {
   die("Error: Please fill in all fields.");
}

$stmt = $conn->prepare("INSERT INTO travel_pass 
                        (driver_id, vehicle_id, cashier_id, card_color_id, total_passengers, total_fare, travel_date, route_id) 
                        VALUES (?, ?, ?, ?, 0, 0.00, NOW(), ?)");
if (!$stmt) {
