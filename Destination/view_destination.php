                    ...
                    <div style="height: 500px;" class="card-body pt-1">
                        <div class="row rounded-3 border border-1 border-gray shadow  d-flex align-items-center mx-0 p-3 mb-3">
                            <div class="col col-3 d-flex align-items-center">
                                <h1 class="fs-4 pt-2">DESTINATION | LISTS</h1>

                            </div>

                            <div class="col col-6 d-flex gap-2  ps-5">
                                <form method="GET" action="view_destination.php" class="d-flex gap-2 ps-5">
                            ...
                            <div class="col col-3 text-end">
                                <a class="btn btn-success border border-1 border-dark fw-semibold" href="add_destination.php">ADD
                        ...
                        <div style="max-height: 400px; overflow-y: auto;">
                            ...
                            <?php
                            try {
                                $sql = "SELECT destination.destination_id, destination.destination_name, 
                                IFNULL(route.route, 'No Route Assigned') AS route,
                                IFNULL(route.fare, 'N/A') AS fare 
                                FROM destination 
                                LEFT JOIN route ON destination.route_id = route.route_id";


                                if (isset($_GET['query']) && !empty($_GET['query'])) {
                                    $search = $conn->real_escape_string($_GET['query']);
                                    $sql .= " WHERE 
                                       destination_name LIKE '%$search%'";
                                }
                                        ...
                                        <ul class="list-group mb-1">
                                            <li class="list-group-item d-flex align-items-center justify-content-between border border-2">
                                                <div class="d-flex align-items-center">
                                                    ...
                                                    <div>
                                                        <span><strong>Name : </strong><?php echo $row['destination_name']; ?></span><br>

                                                    </div>
                                                </div>
                                                <div class="group-btn">
                                                    <a href="edit_destination.php?id=<?php echo $row['destination_id']; ?>" class="btn"><i class="btn btn-outline-success bi bi-pencil-square"></i></a>
                                                    <a href="../Operations/op_delete_destination.php?id=<?php echo $row['destination_id']; ?>" class="btn"><i class="btn btn-outline-danger bi bi-trash"></i></a>
                                                </div>
                                            </li>
                                        </ul>
                            <?php
